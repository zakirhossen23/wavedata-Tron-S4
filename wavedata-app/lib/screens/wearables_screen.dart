...
class _WearablesScreenState extends ConsumerState<WearablesScreen> {
  ...
  Future<void> GetBloodRate() async {
    ...
    var url = Uri.parse(
        'https://wavedata-tron-s4-api.netlify.app/api/GET/Wearable/customAPI?userid=${userid}&url=https://api.und-gesund.de/v5/dailyDynamicValues&token=${this.AccountTokenAddress}&body_startDay=${(new DateTime(2023, 01, 11)).toLocal()}&body_endDay=${(new DateTime(2023, 01, 17)).toLocal()}&body_valueTypes=3001');
    final response = await http.get(url);
    var responseData = json.decode(response.body);
